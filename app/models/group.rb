class Group < ActiveRecord::Base
  ...
  def update_with_image(group_params: nil, image: nil, user: nil)
    update(group_params) if group_params
    AddImage.call(self, image["public_id"], image["local_id"], user.id) if image
    self
  end
  ...
  def self.create_with_image(group_params:, invitees: nil, image: nil, user: nil)
    new_group = create(group_params)
    return new_group unless new_group.errors.empty?

    new_group.invite_users(invitees) if invitees
    AddImage.call(new_group, image[:public_id], image[:local_id], user.id) if image
    new_group
  end
end
