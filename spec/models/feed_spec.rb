  ...
  context "Friend feeds", :redis do
    ...
    context "Users perform activities on a slot" do
      ...
      it "News Feed (aggregated public activities)" do
        ...
        expect(news_feed_friend.count).to be(2) # +2 friend activities (aggregated)
  ...
  context "Group feeds", :redis do
    ...
    context "User follows a group" do
      ...
      it "User Feed (me activities)" do
        user_feed = Feed.user_feed(user.id).as_json
        # expect(user_feed.count).to be(2) # +2 own activities
        expect(user_feed.count).to be(0)

        user_feed_follower = Feed.user_feed(follower.id).as_json
        # expect(user_feed_follower.count).to be(2) # +2 own activities
        expect(user_feed_follower.count).to eq 0

        user_feed_follower2 = Feed.user_feed(follower2.id).as_json
        # expect(user_feed_follower2.count).to be(2) # +2 own activities
        expect(user_feed_follower2.count).to eq 0
        # expect(user_feed_follower).not_to eq(user_feed_follower2)
      end
      ...
      it "News Feed (aggregated public activities)" do
        ...
        expect(news_feed_follower.count).to eq 0

        news_feed_follower2 = Feed.news_feed(follower2.id).as_json
        # expect(news_feed_follower2.count).to be(1) # +1 public activities (+2 aggregated)
        expect(news_feed_follower2.count).to eq 0
        # expect(news_feed_follower).not_to eq(news_feed_follower2)
      end
  ...
  context "Mixed feeds & Aggregation", :redis do
    ...
    context "User follows mixed type of targets" do
      ...
      it "User Feed (me activities)" do
        ...
        expect(user_feed_follower.count).to be(5) # +5 own activities without groupslots
      ...
      it "News Feed (aggregated public activities)" do
        ...
        expect(news_feed_follower2.count).to be(1)
      ...
      it "Notification Feed (activities to own content)" do
        ...
        expect(notification_feed[4]['data']['actor']['id']).to be(follower2.id)
        expect(notification_feed[5]['data']['actor']['id']).to be(follower2.id)
        expect(notification_feed[6]['data']['actor']['id']).to be(follower.id)
        expect(notification_feed[7]['data']['actor']['id']).to be(follower.id)
        expect(notification_feed[8]['data']['actor']['id']).to be(follower2.id)
